...
public class InteractionSystem {
    ...
    private static void handlePlateStorage(Chef chef, PlateStorage storage) {
        ...
        if (chefItem instanceof Plate plate) {
            if (plate.isClean()) {
                storage.returnCleanPlate(plate);
        ...
        if (chef.hasItem() && !station.hasItem()) {
            if (chefItem instanceof Ingredient ingredient) {
                if (ingredient.canBeChopped()) {
                    Ingredient processed = new Ingredient(ingredient.getName(), IngredientState.CHOPPED);
        ...
        if (chef.hasItem() && !station.hasItem()) {
            if (chefItem instanceof Ingredient ingredient) {
                if (ingredient.canBeCooked()) {
                    Item processed = station.process(ingredient);
                    if (processed != null) {
                        station.setItemOnStation(processed);
                        chef.removeInventory();
    ...
    private static void handleServingCounter(Chef chef, ServingCounter counter, OrderManager orderManager) {
        ...
        if (success) {
            ...
            PlateStorage plateStorage = findNearestPlateStorage(chef, counter);
            if (plateStorage != null) {
                plateStorage.returnDirtyPlate(plate);
            }
    ...
    private static PlateStorage findNearestPlateStorage(Chef chef, ServingCounter counter) {
        return null;
    }
